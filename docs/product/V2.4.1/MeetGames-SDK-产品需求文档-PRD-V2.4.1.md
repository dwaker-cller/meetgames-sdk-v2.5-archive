...
# MeetGames-SDK-产品需求文档-PRD-V2.4.1

**PRD：MeetGames SDK 配置、组合包生成与运营 V2.4.1**

## 1. 文档定位与使用规则

### 1.1 文档定位

本文档是 MeetGames SDK 管理工具 V2.4.1 的 Demo 前端等价实现规格，描述前端研发与测试需要复现的页面行为、状态、字段、校验、异常和核心验收场景。

## 2. 产品概述

### 2.1 背景

MeetGames SDK 管理工具面向游戏发行场景，帮助团队按游戏维护可复用的 SDK 接入参数，并针对不同发行渠道选择实际需要的 SDK 能力，生成可交付的渠道组合包，再对组合包中已经包含的能力配置运行策略。<br>当前核心流程和主要交互已趋于稳定，本 PRD 将把 Demo 已确认行为转换为可开发、可测试的前端规则；Demo 未呈现的内容只记录为未覆盖项，不从旧资料中补齐。
...
5. 同一游戏、同一发行渠道允许连续生成多个组合包，需要使用独立包 ID 和生成时间准确区分每条记录。
6. 新包需要继承最近一次生成的同游戏、同发行渠道包的运营配置快照，并在继承后与来源包互不联动。
...
| 同渠道再次生成 | 在已有同渠道包时继续生成新的独立组合包 | 渠道保持可选，显示已有包数量；新表单继承最近同渠道包的运营配置快照，提交后新增列表行 |
...
1. 清晰分离配置中心、组合包生成和已有组合包运营配置的职责。
2. 支持按游戏复用七类 SDK 能力参数，并允许安全保存半成品。
3. 按当前发行渠道准确计算最小配置单元的完整度和可用性。
4. 保证组合包只包含用户实际选择且当前页面允许选择的能力或平台。
5. 保证每次生成后的列表信息、生成时间与下载 ZIP 内容一致。
6. 支持同一游戏、同一发行渠道维护多个相互独立的组合包记录。
7. 新包继承最近同渠道包的运营配置快照，并在生成后与所有其他包互不联动。
...
- 单渠道组合包的连续新建、独立包 ID、生成时间与成功后直接下载。
- 一个游戏、一个发行渠道可对应多个用户可见且相互独立的组合包。
- 新建草稿、同渠道最近包运营配置快照继承和各包独立保存状态。
...
| 组合包列表 | 按当前游戏展示发行渠道、SDK 能力、生成时间和操作；同渠道允许多行，按生成时间倒序排列；行操作为“下载／运营配置” |
| 新建组合包 | 当前支持 App Store、Google Play、ONE Store；已有同渠道包时仍可继续生成，并显示已有包数量 |
| 新包表单 | 只有一个按渠道格式校验的“包名”字段；每次合法提交都创建新的独立组合包记录，不修改已有记录 |
| 能力选择 | 配置完整不等于进入组合包；归因和广告平台允许取消到零选，半成品不能作为有效能力入包 |
| 生成结果 | 合法提交后新增带唯一包 ID 和独立生成时间的列表行，并立即开始下载带生成时间的 ZIP；当前没有异步构建状态 |
...
| 默认策略 | 允许在当前目标包已有其他策略时删除；每个包任意时刻至少保留一个策略。删除默认策略后不再回退到全球，仅该包剩余策略已选择的国家／地区参与发行 |
...
| 包名 | 本次新组合包的名称字段；按渠道分别采用 iOS Bundle ID 或 Android Application ID 的格式提示与校验；包名不是记录唯一键 | “生成SDK”抽屉只显示一个“包名”字段 |
| 组合包记录 | 当前游戏在某一发行渠道下的一次独立生成结果；同渠道允许存在多条记录 | 每行使用内部不可变包 ID 绑定操作；用户通过包名与唯一生成时间区分记录 |
| 包 ID | 新建草稿初始化时分配的内部不可变标识；提交成功后原样持久化 | 列表行、运营配置存储和下载操作按包 ID 定位；当前界面不要求直接显示完整 ID |
| 生成时间 | 组合包成功生成时写入的唯一时间戳，也是用户可见和下载制品的追溯键 | 列表显示到毫秒；ZIP 文件名及 `manifest.json.generatedAt` 使用同一值 |
| 最近同渠道包 | 当前游戏、当前发行渠道下 `generatedAt` 最大的已生成组合包 | 再次选择已有包的渠道时，页面以该包作为运营配置继承来源 |
| 运营配置快照 | 生成新包时从最近同渠道包深拷贝到新草稿的国家策略组及运行配置 | 新草稿使用新的包 ID；复制完成后来源包、新包及其他包互不联动 |
...
| 生成结果 | 一次提交校验通过后，页面新增组合包列表行并开始下载 ZIP 的可观察结果 | 每次生成均产生新的包 ID 和生成时间，不覆盖已有行 |
| 新建内嵌运营草稿 | 新建组合包尚未生成时维护的运营策略 | 位于“生成SDK”抽屉内，属于新建生成草稿 |
| 已有组合包运营配置 | 依附于已生成渠道包、可独立保存的运营策略 | 渠道包列表行内的“运营配置”入口 |
| 渠道允许范围 | 当前目标包发行渠道可供策略组选择的国家／地区全集 | 添加策略组时的筛选数量依据此集合计算；“已占用”只统计当前包内其他策略 |
| 有效发行范围 | 当前目标包实际有策略承接的国家／地区集合 | 默认策略存在时等于渠道允许范围；默认策略删除后等于当前包现存自定义策略组国家的并集 |
| 默认策略 | 每个组合包初始存在的兜底策略；存在时承接该包内所有未被自定义策略组占用的国家／地区 | 允许删除，但每个包任意时刻必须至少保留一个策略 |
| 策略组 | 由策略名称、一个或多个在当前包内未被占用的国家／地区及独立运行配置组成的区域策略 | “添加策略组”弹窗和当前包策略列表；同一国家可同时属于其他包的策略 |
| 当前编辑策略 | 运营抽屉中当前目标包正在修改的默认策略或自定义策略组 | 策略项显示“当前编辑”；不得跨包共享编辑状态 |
...
| 状态 | 判定 | Demo 表现 |
|---|---|---|
| `included` | 当前生成表单中，用户选择了一个 `available` 的简单能力或完整平台 | 简单能力为勾选态；平台按钮为选中态；计入“已选择 N / 7 项能力” |
...
| 状态 | 进入条件 | 后续行为 |
|---|---|---|
| `editing` | 打开新建抽屉，正在填写或修改 | 可继续编辑或提交；存在修改后请求退出时才提供保存草稿分流 |
| `saved-draft` | 关闭有修改的抽屉并选择“保存草稿” | 返回列表；同一页面会话中再次进入可恢复并继续编辑 |
| `validation-blocked` | 点击“生成SDK”后，未选择发行渠道、包名为空／不符合当前渠道格式／去除首尾空格后超过 150 个字符、未选择任何 SDK 能力，或已选能力因配置变化而不再可用 | 抽屉保持打开并保留当前输入，显示汇总提示及字段／能力错误；不形成 `saved-draft`，不新增列表行，也不触发下载；修改后可再次提交 |
| `generated` | 提交校验通过 | 创建新的唯一包 ID，记录独立生成时间，新增列表行并开始下载 ZIP；已有列表行保持不变 |
...
选择发行渠道时，系统先为新草稿分配不可变的候选包 ID。若当前游戏已有同渠道包，必须按 `generatedAt DESC, packageId DESC` 找到最近生成的一条，只读取该来源包最近一次成功保存的运营配置快照，将快照深拷贝到新草稿并绑定候选包 ID；来源包尚未保存的 `dirty` 值不得被继承。继承只发生在新草稿首次初始化时；恢复已保存草稿时不得因为后来出现了更新的同渠道包而重新选源或覆盖用户修改。生成成功后持久化同一候选包 ID，各组合包的运营配置独立保存，修改任意包不得联动其他包。当前渠道没有历史包时，使用默认运营配置初始化。
...
| 已选渠道且存在历史同渠道包 | 配置完整度按当前游戏参数判断 | 当前渠道可正常选择 | 运营配置继承最近同渠道包快照 | 新草稿使用独立包 ID；继承后与来源包互不联动 |

### 7.5 关键流程图

#### 7.5.1 配置中心到新建渠道包

![流程图](assets/PRD-V2.4.1/flow-01.png)

#### 7.5.2 配置完整、渠道可用与入包选择

![流程图](assets/PRD-V2.4.1/flow-02.png)

#### 7.5.3 同渠道多包生成与运营配置继承

```mermaid
%% MEANING: 展示同游戏同渠道再次生成时，运营配置如何继承并在生成后保持包间独立
%% Type: flowchart
%% Direction: TD，突出“是否存在同渠道历史包”和“提交是否通过”两个决策点
flowchart TD
    open["打开生成 SDK"]
    channel["选择发行渠道"]
    allocate["为新草稿分配不可变候选包 ID"]
    history{"该游戏与渠道是否已有组合包"}
    defaults["使用默认运营配置初始化新草稿"]
    latest["按 generatedAt 找到最近同渠道包"]
    inherit["深拷贝运营配置快照并绑定候选包 ID"]
    edit["填写包名、选择能力并调整新包运营草稿"]
    validate{"提交校验是否通过"}
    blocked["保留输入并显示字段或能力错误"]
    create["新增独立列表行，记录 generatedAt 并下载 ZIP"]
    isolated["各组合包运营配置独立保存，后续互不联动"]

    open --> channel --> allocate --> history
    history -->|"否"| defaults --> edit
    history -->|"是"| latest --> inherit --> edit
    edit --> validate
    validate -->|"否"| blocked --> edit
    validate -->|"是"| create --> isolated

    classDef info fill:#cce5ff,stroke:#004085,color:#004085
    classDef success fill:#d4edda,stroke:#155724,color:#155724
    classDef warning fill:#fff3cd,stroke:#856404,color:#856404
    class open,channel,edit info
    class defaults,latest,inherit warning
    class create,isolated success
```
...
配置中心用于在当前游戏上下文中维护七类 SDK 能力参数。配置中心负责参数录入、完整度判断、配置摘要、草稿保存、未保存保护和已有渠道包情况下的风险提示；是否把能力或平台放入某次渠道包，仍由“生成SDK”流程决定。
...
| FR-CFG-028 | “保存修改”必须按 8.5 的完整／草稿规则保存，关闭提示和能力配置弹窗，并显示对应反馈。 |
| FR-CFG-029 | 点击普通未保存提示的遮罩不得关闭提示，也不得隐式选择保存或放弃。 |

### 8.7 当前游戏已有渠道包时的线上风险确认
...
| FR-CFG-033 | “确认风险并保存”必须按 8.5 的完整／草稿规则保存并关闭两层弹窗；该操作只代表保存，不得显示或推导独立发布成功。 |
...
协议与隐私不使用 8.7 的最终保存风险弹窗。当前游戏至少有一个渠道包包含协议与隐私能力时，添加、编辑或删除协议必须在动作执行前分别显示：
...
| FR-CFG-037 | 用户已对本次协议动作确认风险后，配置卡最终保存或关闭时不得再次显示 8.7 的通用风险确认。 |
...
选择发行渠道后，生成抽屉中不满足当前渠道可用条件的能力显示“待配置”和“前往配置”。点击后，在生成抽屉上方打开快捷配置弹窗。
...
| 规则编号 | 功能要求 |
|---|---|
| FR-CFG-042 | 快捷配置保存后必须停留在原生成抽屉，并立即重新计算能力摘要、可用性及选择计数。 |
...
## 9. 能力规格 A：登录、支付、协议与隐私、合规

### 9.1 本部分适用规则

本部分定义登录、支付、协议与隐私、合规四类 SDK 能力。各能力均继承第 7 节状态模型及第 8 节通用保存、关闭和风险确认规则；本节只补充能力独有的字段、完整度、当前渠道可用性、入包内容和风险文案。
...
| 规则编号 | 功能要求 |
|---|---|
| FR-LOGIN-010 | Guest 单独存在时，登录能力必须保持不完整，保存结果按草稿处理；当页面展示登录完整度错误时，提示“请至少再添加一种登录方式”。普通草稿保存关闭弹窗后以第 8.5 节草稿反馈为准。 |
...
登录参数变更的详细风险说明为“可能导致线上玩家登录失败、账号绑定或回调异常。”配置中心与快捷配置的风险弹窗内容和操作分别遵循第 8.7 节与第 8.9.3 节。当前“批量导入”没有可观察结果，不纳入本期登录功能。
...
| FR-AGR-012 | 当前游戏已有包含协议与隐私能力的渠道包时，配置中心的添加、编辑、删除必须在动作执行前使用第 8.8 节专项风险确认；确认一次后外层保存不重复提示通用风险。 |
| FR-AGR-013 | 专项风险确认中的“取消”或右上角关闭不得执行协议动作；确认删除后直接从当前外层表移除目标分组，不再叠加第二个删除确认。 |
| FR-AGR-014 | 快捷配置复用相同全局协议分组和字段规则；其保存与退出风险分流遵循第 8.9.3 节，不得假设快捷入口与配置中心动作前提示完全相同。 |
...
FAQ 语种固定为简体中文、繁体中文、英语、韩语、阿拉伯语。背景图提示支持 jpg/png、横屏建议 1920×1080、竖屏建议 1080×1920、≤3MB.
...
| FR-SUP-004 | 生成页面只允许选择整个客服工具能力，不提供四个子模块的独立入包开关；可用后默认整体入包，用户可取消。 |
...
客服子模块不存在独立启用状态；完整度、摘要和渠道可用性只依据本节定义的可见字段计算。无论当前组合包是否包含客服工具，运营配置均不展示客服策略或客服参数；客服只能在配置中心维护，并在生成时作为整个能力选择。

## 11. 组合包生成与同渠道多包管理

### 11.1 列表与新建

生成及运营中心按当前游戏展示组合包。列表列为“发行渠道／SDK 能力／生成时间／操作”，渠道单元同时显示包名；同一渠道允许出现多行，列表按 `generatedAt` 从新到旧排列。行操作为“下载／运营配置”。空态为“当前游戏暂无 SDK 组合包。”

| 规则编号 | 功能要求 |
|---|---|
| FR-GEN-001 | “生成SDK”抽屉继承列表当前游戏且不允许在抽屉内切换游戏；渠道固定按 App Store、Google Play、ONE Store 展示。 |
| FR-GEN-002 | 当前游戏已有组合包的渠道不得禁用；渠道卡显示“已有 N 个包，可继续生成”。即使三个渠道均已有包，“生成SDK”入口仍保持可用。 |
...
| FR-GEN-006 | 新建表单选择能力后展示运营配置区；若当前游戏存在同渠道历史包，运营配置区以最近生成包的运营配置快照初始化，否则使用默认值。 |
| FR-GEN-007 | 合法提交不显示二次确认：持久化草稿初始化时分配的候选包 ID，抽屉关闭、列表新增独立行并立即发起 ZIP 下载。`generatedAt` 必须取 `max(当前时间, 当前游戏已有最大 generatedAt + 1ms)`，确保时钟回拨或同毫秒提交时仍保持唯一、单调递增。已有组合包不得被覆盖或更新。 |
...
| 规则编号 | 功能要求 |
|---|---|
| FR-GEN-011 | 同一游戏、同一发行渠道允许存在多个组合包；每次点击“生成SDK”并成功提交，都必须新增一条记录，不得修改、替换或复用已有包记录。 |
| FR-GEN-012 | 选择渠道并初始化新草稿时必须分配唯一、不可变的候选包 ID，提交成功后持久化同一 ID。包名允许与同渠道其他包相同，包名不得作为列表记录、运营配置存储或下载结果的唯一键。 |
| FR-GEN-013 | 选择发行渠道时，系统必须在当前游戏下筛选同渠道历史包，并按 `generatedAt DESC, packageId DESC` 选择最近生成的一条作为运营配置继承来源。只允许读取来源包最近一次成功保存的快照，不得读取尚未保存的 `dirty` 值。当前渠道没有历史包时使用默认运营配置。 |
| FR-GEN-014 | 继承必须复制来源包的国家策略组及包内登录、协议、合规运行项，并将复制结果绑定新草稿的候选包 ID。策略结构可以复制，但有效运行项必须与新包本次实际选择且可用的能力、登录方式和协议分组取交集；新包不存在的能力或子项不得显示、生效或输出，也不保留可在未来自动恢复的隐藏运行值。能力选择和配置中心参数仍按当前生成表单与当前游戏配置计算，不从来源包强制覆盖。 |
| FR-GEN-015 | 运营配置继承是一次性快照复制。生成后，来源包、新包及其他同渠道包必须分别存储；修改或保存任意一个包不得联动修改其他包。 |

### 11.4 退出、草稿与下载

| 规则编号 | 功能要求 |
|---|---|
| FR-GEN-016 | 新建表单存在未保存修改时，关闭或返回列表必须显示“生成配置尚未完成”，操作为“继续编辑／不保存／保存草稿”。 |
| FR-GEN-017 | 保存草稿后返回列表；同一页面会话再次进入时恢复原候选包 ID、继承来源、已复制快照和用户修改。即使期间生成了更新的同渠道包，也不得重新继承或覆盖已保存草稿。刷新后草稿丢失，列表不显示草稿标记。 |
| FR-GEN-018 | “不保存”只丢弃本轮未保存的新包表单和内嵌运营草稿；不得修改任何已有组合包。已保存的快捷配置仍按配置中心规则保留。 |
| FR-GEN-019 | 生成下载文件名为 `MeetGames-{游戏安全名}-{渠道安全名}-{yyyyMMddHHmmssSSS}-SDK.zip`；游戏／渠道中的空白和文件名非法字符转为连字符并合并连续连字符。17 位生成时间必须对应当前包的 `generatedAt`，用于区分同渠道多包下载。 |
| FR-GEN-020 | 列表每行提供独立“下载”入口。下载现有包时不得改变该包的 `generatedAt`；ZIP 内 `manifest.json` 的 `generatedAt` 必须等于列表记录时间，并额外记录本次 `downloadedAt`。 |
| FR-GEN-021 | 列表按 `generatedAt DESC, packageId DESC` 稳定排序。每行“下载／运营配置”事件和对应 DOM 数据必须绑定内部不可变包 ID，不得按渠道或包名查找目标记录；完整包 ID 无需在当前界面直接展示。 |
| FR-GEN-022 | `generatedAt` 是当前 Demo 下载制品的公开追溯键：列表显示值、ZIP 文件名时间段和 `manifest.json.generatedAt` 必须完全一致。内部包 ID 不要求写入当前 Demo 的 manifest。 |
...
选择已有同渠道包时，新建内嵌运营草稿以最近生成包的运营配置快照初始化；当前渠道没有历史包时使用默认值。两种运营上下文复用相同策略字段，但保存、退出和状态提示分别遵循生成流程与独立运营流程。<br>页面顶部“发行范围”展示有效发行范围及其国家／地区数量；添加策略组中的国家候选、洲筛选及数量始终依据渠道允许范围计算。

所有国家占用、策略复制、删除回落、有效发行范围和未保存状态均以当前目标包 ID 为作用域。同一游戏、同一发行渠道的不同组合包可以分别使用同一个国家／地区；在包 A 中添加、删除或保存策略不得改变包 B 的占用、筛选、发行范围或运行配置。
...
| 规则编号 | 功能要求 |
|---|---|
| FR-OPS-001 | 运营配置只能展示并维护本次新建选择或当前目标包已有的登录、协议与合规策略，不得增加构建能力集合中不存在的能力或参数；继承值必须按新包实际能力及可用子项取交集，不得展示、增加或修改客服工具及其四个子模块。 |
...
“添加策略组”模态覆盖当前运营上下文。策略名称默认生成“策略1”“策略2”……，去除首尾空格后必填，最长 30 个字符；当前不校验重名。创建时至少选择一个当前渠道允许范围内且未被当前目标包其他自定义策略占用的国家／地区。

| 规则编号 | 功能要求 |
|---|---|
| FR-OPS-005 | 国家筛选固定为全部、亚洲、欧洲、北美洲、南美洲、大洋洲、非洲，并显示当前渠道允许范围内的国家数量；数量包含已被当前目标包其他策略占用的国家，但不得统计其他组合包的占用。 |
| FR-OPS-006 | 搜索必须支持中文名、英文名和两位代码，大小写不敏感并按包含关系匹配；国家卡片只显示中文名。 |
| FR-OPS-007 | 已被当前目标包其他策略占用的国家必须排在当前结果末尾、禁止选择，并显示“已分组”和占用策略名；其他包中的同一国家不得在本包显示为已占用。 |
| FR-OPS-008 | 左侧提供“全选当前筛选／清空”；右侧已选面板只允许逐项移除，不提供整体清空。 |
| FR-OPS-009 | 真实零结果时显示“没有匹配的国家/地区”和“请尝试其他搜索词或区域筛选。” |
| FR-OPS-010 | 名称为空或零选时禁止创建并显示对应状态；满足条件后在当前目标包内创建策略并设为当前编辑。该包默认策略存在时复制该默认策略；默认策略已删除时复制该包当前活动策略。 |
...
| FR-OPS-016 | 选择已有同渠道包时，新建内嵌运营草稿必须从当前游戏、当前渠道下 `generatedAt` 最大的组合包复制运营配置；不得从其他渠道或更早包继承。 |
| FR-OPS-017 | 复制结果必须使用新草稿的包 ID，并与来源包使用独立存储。继承完成后，修改来源包、新草稿或任意其他包均不得联动修改其余对象。 |
| FR-OPS-018 | 仅对当前目标包允许执行的策略删除显示二次确认。该包只剩一个策略时不进入确认，直接阻止删除并提示“至少保留一个发行策略。”；该包存在其他策略时允许删除默认策略。默认策略存在时，删除区域策略后该组国家仅在当前包回落默认策略；默认策略已删除时，该组国家仅退出当前包有效发行范围。 |
| FR-OPS-019 | 最近同渠道包已保存登录、协议或合规运营策略时，新建草稿继承对应快照；生成后重新打开新包运营配置必须显示该新包自己的保存结果，后续来源包变化不得覆盖。 |
...
| FR-OPS-023 | 当前目标包默认策略存在时，其有效发行范围等于渠道允许范围；默认策略删除后，该包有效发行范围等于该包现存自定义策略组国家的并集。页面顶部“发行范围”及数量按当前包有效发行范围更新；添加策略组的候选国家和洲筛选数量仍按渠道允许范围计算。 |

默认策略允许删除，但每个组合包任意时刻至少保留一个策略。默认策略删除后不再提供国家回退，当前包有效发行范围仅为该包现存自定义策略组国家的并集；未被当前包任何现存策略组覆盖的国家不发行。其他组合包的策略与发行范围不受影响。
...
| 已选能力失效 | 已选择的能力因配置变化而不再满足当前渠道可用条件 | 点击“生成SDK” | 保持抽屉和当前输入，提示重新确认能力选择；不新增列表行，不下载 |
| 新建成功 | 新建表单校验通过 | 点击“生成SDK” | 不出现二次确认；持久化草稿候选包 ID，新增独立列表行、写入按单调规则生成的唯一时间并开始下载 ZIP；已有行不变 |
| 同渠道首包默认值 | 当前游戏、当前渠道没有历史组合包 | 选择该渠道 | 不显示继承提示，使用默认运营配置初始化新草稿 |
| 同渠道继续生成 | 当前游戏的某发行渠道已有一个或多个组合包 | 再次打开生成抽屉并查看该渠道 | 渠道保持可选，显示“已有 N 个包，可继续生成”；即使三个渠道均已有包，生成入口也不禁用 |
| 最近包快照继承 | 同游戏、同渠道存在多个历史包，最近包与更早包的运营配置不同 | 选择该渠道开始生成新包 | 只复制 `generatedAt` 最大包的运营配置快照；新草稿使用独立包 ID，更早包和来源包均不被修改 |
| 继承能力交集过滤 | 最近同渠道包包含登录 A、登录 B 和协议 P；新包仅选择且可用登录 A，不选择协议 | 继承运营配置并生成新包 | 新草稿只展示、继承和输出登录 A 的有效运行项；登录 B 与协议 P 不显示、不生效、不输出，也不作为隐藏值保留供未来恢复；来源包保持不变 |
| 跨渠道继承隔离 | 当前游戏仅 Google Play 存在历史包 | 选择 App Store 或 ONE Store | 不跨渠道继承 Google Play 运营配置，不显示继承提示，使用默认运营配置初始化 |
| 继承后互不联动 | 新包已经从最近同渠道包继承并生成 | 分别修改并保存来源包、新包或其他同渠道包 | 每次修改只影响目标包；其他包的策略与运行项保持原值 |
| 同包名多包 | 同游戏、同渠道已有包名 X | 使用相同包名 X 再次生成 | 允许生成；新包使用不同内部包 ID 和唯一 `generatedAt`，旧包保持不变 |
| 多包标识与排序 | 同游戏、同渠道连续生成多个包，包含同毫秒请求或本地时钟回拨 | 查看组合包列表 | 候选包 ID 唯一；`generatedAt` 按 `max(now, currentMax+1ms)` 保持唯一递增；列表按 `generatedAt DESC, packageId DESC` 稳定排序，同渠道多行均保留 |
| 生成草稿退出分流 | 新建表单存在未保存修改 | 关闭或返回列表 | “继续编辑”保留当前表单；“不保存”丢弃本轮新包表单和内嵌运营草稿但不修改已有包；“保存草稿”后同一页面会话重开恢复，刷新后丢失 |
| 生成草稿继承冻结 | 草稿已继承包 A 并保存；随后同渠道又生成了更新的包 B | 恢复原草稿 | 继续使用包 A 的继承来源、候选包 ID、已复制快照和用户修改，不改为继承包 B |
...
| 已占用国家 | 某国家已被当前目标包其他策略占用 | 查看相同筛选结果 | 该国家排在结果末尾，显示当前包占用策略且不可选择 |
| 跨包国家隔离 | 同渠道包 A 已将国家 X 分配给策略，包 B 未分配 | 打开包 B 的国家选择器并修改策略 | 国家 X 在包 B 仍可选择；修改包 B 不改变包 A 的占用、策略或发行范围 |
| 创建策略组 | 策略名有效且选择至少一个在当前包内未占用的国家 | 创建策略组 | 当前包默认策略存在时复制该默认策略；默认策略已删除时复制当前包活动策略；新策略成为当前包当前编辑 |
...
| 组合包列表 | 当前游戏至少存在一个组合包 | 查看列表 | 仅含“发行渠道／SDK 能力／生成时间／操作”四列；包名显示在发行渠道单元；同渠道允许多行并按生成时间倒序；行操作为“下载／运营配置” |
| 下载结果 | 新建表单校验通过，或列表中已有目标包 | 点击“生成SDK”或目标行“下载” | 文件名包含目标包 17 位生成时间；`manifest.json.generatedAt` 与该行一致，`downloadedAt` 记录本次下载时间；下载不得修改原生成时间 |
| 登录策略继承 | 最近同渠道包已保存登录策略，且新包选择登录能力 | 选择渠道并生成新包后打开运营配置 | 新草稿继承最近包的登录选择与顺序；新包保存后使用独立快照，后续来源包变化不覆盖新包 |
| 删除默认策略 | 当前包默认策略和至少一个自定义策略同时存在 | 确认删除默认策略 | 当前包默认策略被删除，至少保留一个自定义策略；当前包发行范围数量更新为该包现存自定义策略覆盖国家的去重数量，其他国家不发行；其他包不受影响 |
| 禁止删除最后策略 | 当前包只剩一个策略，无论其是否为默认策略 | 尝试删除 | 不进入删除确认，不删除该策略，并提示“至少保留一个发行策略。” |
| 删除自定义策略 | 当前包删除目标策略时仍存在其他策略 | 确认删除 | 当前包默认策略存在时，目标国家仅在当前包回落默认策略；默认策略已删除时，目标国家仅退出当前包有效发行范围；其他包不受影响 |
...
| 文档版本 | 日期 | 状态 | 变更 |
|---|---|---|---|
| v1.0 | 2026-08-18 | final | 基于 V2.4 内容确认 V2.4.1 文档版本。 |
| v1.1 | 2026-08-18 | final | 与运行标记 `20260818-v24-multi-package-55` 对齐：同游戏同渠道允许多个组合包；新包继承最近同渠道包运营配置快照；各包以唯一 ID 和生成时间区分并在继承后互不联动；同步更新下载、列表与验收规则。 |
